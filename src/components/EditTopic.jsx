...
export default function EditTopic({data}) {
    ...
    const [course,setcourse] =useState({title:data.course.title,description:data.course.description,chapters:data.course.chapters})
     ...
     const handleUpdate =async(e)=>{
       ...
       console.log(course)
        try {
            ...
            const res = await axios.put(`/api/course/${id}`,course)
            console.log(res)
            router.push("/")
    ...
    <div> 
        ...
        />
        {session?(
        <form onSubmit={handleUpdate} className='flex flex-col justify-center items-center mt-7 border shdaow-md py-3 border-indigo-600'>
         ...
         <button type='submit' className='py-2 px-3 ml-2 mr-5 mt-2 bg-indigo-600 rounded-md hover:bg-indigo-800' onClick={()=>setcourse({...course,email:session.user.email})}>Update</button>
          </div>
